...
For the Cloudflare/Akamai to Varnish/HAProxy/Coraza path, inventory can include `dnsRecords`, `wafRules`, `cacheRules`, `redirects`, `origins`, `tlsSettings`, `botRules`, and `pageRules`. The live Cloudflare collector currently populates DNS records, WAF/custom/managed ruleset metadata, cache rules, dynamic redirects, inferred origins, TLS settings, bot-related rules, and page rules; Akamai is fixture-only. WAF entries contain rule metadata and expressions only; they should not contain provider credentials.

For the OpenAI/Anthropic to vLLM/LiteLLM path, inventory can include `aiModelUsageClasses`, `aiTokenVolumes`, `aiLatencyExpectations`, `aiSensitivePromptCategories`, `aiToolUsages`, and `aiFallbackBehaviors`. The live OpenAI collector currently populates model usage classes and token volumes from aggregate usage, and can attach owner, latency, streaming, and fallback metadata supplied as CLI flags. Anthropic remains fixture-only. Entries contain operational metadata only; OpenExit should never collect provider credentials or raw prompts.
